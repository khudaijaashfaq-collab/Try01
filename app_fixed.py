...
def get_usage_db_path():
    """Lightweight prototype analytics store.

    On Streamlit Community Cloud this persists while the current app instance is alive,
    but can reset after redeploys/restarts. For production analytics, replace this with
    a hosted database or analytics service.
    """
    return os.path.join(os.getcwd(), "viza_pilot_usage.db")


def register_visit_once():
    if st.session_state.get("visitor_registered"):
        return
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS visits (id INTEGER PRIMARY KEY AUTOINCREMENT, visited_at TEXT NOT NULL)"
            )
            conn.execute(
...
def add_token_usage(response):
    ...
    bucket = st.session_state.token_usage
...
def requirement_status(req):
    ...
    # A requirement is complete only when there is a currently confirmed
    # document whose selected type satisfies that requirement.
    for doc in st.session_state.documents:
        if doc.get("confirmed") and doc.get("doc_type") in accepted_types:
            return "complete"
...
DOC_TYPE_RULES = {
    "passport": {
        "strong": ["passport no", "passport number", "type p", "issuing authority", "republic of pakistan"],
        "weak": ["passport", "place of birth", "nationality", "date of issue", "date of expiry"],
        "filename": ["passport"],
    },
    "cnic": {
        "strong": ["computerized national identity card", "national identity card", "cnic", "nadra"],
        "weak": ["identity card"],
        "filename": ["cnic", "nicop", "identity"],
    },
    "admission_letter": {
        "strong": ["letter of admission", "offer of admission", "admission letter", "we are pleased to offer", "admitted to"],
        "weak": ["university", "programme", "program", "enrolment", "enrollment", "semester"],
        "filename": ["admission", "offer", "zulassung"],
    },
    "degree_certificate": {
        "strong": ["degree certificate", "awarded the degree", "has been conferred", "graduation certificate"],
        "weak": ["bachelor", "master", "graduated", "degree"],
        "filename": ["degree", "certificate", "diploma"],
    },
    "transcript": {
        "strong": ["academic transcript", "transcript of records", "statement of marks"],
        "weak": ["credits", "semester", "grade", "cgpa", "gpa", "course code"],
        "filename": ["transcript", "marksheet", "grades"],
    },
    "tuition_payment": {
        "strong": ["tuition fee payment", "payment receipt", "tuition fees paid", "fee receipt"],
        "weak": ["tuition", "fees paid", "payment"],
        "filename": ["tuition", "fee", "receipt"],
    },
    "blocked_account": {
        "strong": ["blocked account", "sperrkonto", "blocked amount", "monthly disposal amount"],
        "weak": ["account holder", "deposit amount"],
        "filename": ["blocked", "sperrkonto", "fintiba", "expatrio"],
    },
    "deed_of_obligation": {
        "strong": ["verpflichtungserklärung", "declaration of commitment", "deed of obligation"],
        "weak": ["sections 66", "sections 68", "sponsor in germany"],
        "filename": ["obligation", "commitment", "verpflichtung"],
    },
    "scholarship": {
        "strong": ["scholarship award", "scholarship letter", "funding award", "stipend"],
        "weak": ["scholarship", "hec", "funding"],
        "filename": ["scholarship", "stipend", "hec"],
    },
    "health_insurance": {
        "strong": [
            "health insurance certificate", "travel health insurance", "certificate of insurance",
            "insurance certificate", "health insurance", "medical insurance", "insured person",
            "policy number", "insurance policy", "medical expenses", "coverage period"
        ],
        "weak": ["insurance", "coverage", "insured", "policy", "medical", "health cover"],
        "filename": ["insurance", "health", "policy", "coverage"],
    },
    "accommodation": {
        "strong": ["tenancy agreement", "rental agreement", "accommodation confirmation", "housing contract", "dormitory confirmation"],
        "weak": ["accommodation", "housing", "tenancy", "rental", "dormitory", "wohnheim", "landlord"],
        "filename": ["accommodation", "housing", "rent", "tenancy", "dorm"],
    },
    "portal_application": {
        "strong": ["consular services portal", "national visa application", "application for a national visa"],
        "weak": ["visa application", "application form"],
        "filename": ["application", "portal", "visa-form"],
    },
    "appointment_confirmation": {
        "strong": ["appointment confirmation", "visa appointment", "booking confirmation"],
        "weak": ["appointment", "booking reference"],
        "filename": ["appointment", "booking"],
    },
}
...
def classify_document(text, filename):
    ...
    for doc_type, rules in DOC_TYPE_RULES.items():
        ...
        # Filename is often a useful signal but should not dominate the document text.
        for hint in rules.get("filename", []):
            if hint in filename_l:
                score += 5.0

        # Strong phrases identify document purpose.
        for hint in rules.get("strong", []):
            if hint in text_l:
                score += 4.0

        # Weak phrases provide supporting evidence only.
        for hint in rules.get("weak", []):
            if hint in text_l:
                score += 1.0
    ...
    # If an insurance document contains passport details, insurance-specific phrases
    # should still win. Conversely, a true passport should have passport-specific markers.
...
def extract_fields(text, doc_type):
    ...
    if doc_type == "health_insurance":
        policy_patterns = [
            r"(?:policy(?: number| no\.?| #)?|certificate(?: number| no\.?))\s*[:\-]?\s*([A-Z0-9\-/]{4,30})",
        ]
        ...
        coverage_patterns = [
            r"(?:coverage|insured period|period of insurance)\s*(?:from)?\s*[:\-]?\s*(\d{1,2}[./\-]\d{1,2}[./\-]\d{2,4})\s*(?:to|until|-)\s*(\d{1,2}[./\-]\d{1,2}[./\-]\d{2,4})",
        ]
        for pattern in coverage_patterns:
            m = re.search(pattern, text, flags=re.I)
            if m:
                fields["coverage_start"] = m.group(1).strip()
                fields["coverage_end"] = m.group(2).strip()
                break
...
def analyze_uploaded_file(uploaded_file):
    ...
    if not text:
        warnings.append(
            "No machine-readable text was extracted. This file appears to be scanned or image-based, so Viza Pilot could not read its text automatically."
        )
    if doc_type == "unknown":
        warnings.append("Viza Pilot could not identify this document confidently. Please choose the correct document type below.")
    ...
    for doc in docs:
        if not doc.get("confirmed"):
            issues.append({
                ...
                "detail": (
                    "The document details have not yet been confirmed by the applicant."
                ),
                "doc_id": doc["id"],
                "priority": 85,
            })
    ...
    for doc in docs:
        if doc.get("confidence") == "low" or doc.get("warnings"):
            issues.append({
                ...
                "detail": " ".join(doc.get("warnings", [])) or "Viza Pilot is not fully confident about this document type.",
                "doc_id": doc["id"],
                "priority": 80,
            })
...
def prioritized_actions():
    ...
    if not actions:
        actions.append({
            ...
            "detail": (
                "No unresolved issues were detected. Reverify time-sensitive "
...
def ask_viza_pilot(question):
    ...
    if not groq_is_configured():
        return (
            ...
            "You can still use the checklist, document review, issue center and readiness features."
        ), results
...
def app_header():
    ...
    st.caption(
        "Your guided visa preparation assistant for Germany student applications"
    )
...
def sidebar():
    with st.sidebar:
        ...
        nav = [
            ...
            ("🔍 Review document details", "review"),
        ...
        st.divider()
        st.caption("App activity")
        st.metric("Visits", get_visit_count())
        st.metric("Tokens used", st.session_state.token_usage.get("total", 0))
...
def screen_upload():
    ...
    st.write(
        "Upload your application documents and Viza Pilot will identify the document type and read the key details."
    )

    st.warning(
        "Your uploaded files are used to review your application during this session."
    )
...
def screen_review():
    st.header("Review document details")
    ...
    st.write(
        "Please review the details Viza Pilot found in each document and confirm or correct them before continuing."
    )
    ...
    for idx, doc in enumerate(st.session_state.documents):
        with st.expander(
            ...
            doc_types = list(DOC_TYPE_RULES.keys()) + ["unknown"]
            ...
            previous_type = doc.get("doc_type")
            doc["doc_type"] = selected_type
            if selected_type != previous_type and doc.get("text_preview"):
                refreshed = extract_fields(doc["text_preview"], selected_type)
                # Keep user-entered values where they already exist, add fields relevant to the corrected type.
                for field_key, field_value in refreshed.items():
                    doc.setdefault("extracted", {}).setdefault(field_key, field_value)
                doc["confirmed"] = False

            st.caption(f"How confident Viza Pilot is about the document type: {doc['confidence']}")
            ...
            editable_keys = [
                ...
                ("coverage_start", "Insurance cover starts"),
                ("coverage_end", "Insurance cover ends"),
            ]

            for key, label in editable_keys:
                always_show = {"name"}
                if doc.get("doc_type") == "passport":
                    always_show.update({"passport_number", "date_of_birth", "expiry_date"})
                elif doc.get("doc_type") == "health_insurance":
                    always_show.update({"policy_number", "coverage_start", "coverage_end"})

                if key in extracted or key in always_show:
            ...
            if st.checkbox(
                "I confirm the document type and details above are correct",
                value=doc["confirmed"],
    ...
    if st.button("Check my documents", type="primary"):
        build_issues()
        go("issues")
...
def screen_issues():
    ...
    if not issues:
        st.success("No unresolved issues were detected by the current checks.")
        st.info(
...
def screen_readiness():
    ...
    if result["blocking_count"] > 0:
        ...
        st.success(
            "Your application appears highly complete based on the checks currently available in Viza Pilot."
        )
    else:
        st.info(
